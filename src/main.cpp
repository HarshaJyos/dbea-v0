...
{
    unsigned int seed = 42;  // default

    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg == "--seed" && i+1 < argc) {
            seed = static_cast<unsigned int>(std::atoi(argv[++i]));
            std::cout << "[MAIN] Using provided seed: " << seed << std::endl;
            break;
        }
    }
    ...
    Config cfg;
    // Tuned parameters — slightly more conservative for very long runs
    cfg.max_beliefs           = 300;          // increased room for long simulation
    cfg.exploration_rate      = 0.92;
    cfg.learning_rate         = 0.08;         // slower learning over many lifetimes
    cfg.belief_learning_rate  = 0.04;
    cfg.belief_decay_rate     = 0.015;        // slightly faster decay
    ...
    cfg.gamma                 = 0.985;
    cfg.merge_threshold       = 0.92;         // more aggressive merging
    cfg.min_beliefs_before_prune = 30;

    // Evolutionary parameters — scaled for longer runs
    cfg.evo_cycle_freq        = 800;          // was 200 — less frequent
    cfg.symbiotic_uplift      = 0.10;
    cfg.symbiosis_prob        = 0.12;

    const int num_lifetimes       = 80;
    const int base_episodes       = 10;
    const int therapy_episodes    = 30;

    // Dynamic placement of special phases
    const int trauma_lifetime     = num_lifetimes / 10;   // ~8th lifetime (early-mid)
    const int therapy_lifetime    = num_lifetimes - 5;    // near the end (mature agent gets therapy)
    ...
    std::cout << "Total lifetimes: " << num_lifetimes 
              << " | Trauma at lifetime " << trauma_lifetime 
              << " | Therapy at lifetime " << therapy_lifetime << "\n\n";
    ...
    {
        // Progress print every 5 lifetimes
        if ((life + 1) % 5 == 0 || life == 0 || life == num_lifetimes-1) {
            std::cout << "\n[Progress] Starting lifetime " << (life + 1) 
                      << " / " << num_lifetimes << "  (beliefs: " << agent.get_belief_count() << ")\n";
        }

        bool is_trauma  = (life == trauma_lifetime);
        bool is_therapy = (life == therapy_lifetime);

        int episodes_this_life = is_therapy ? therapy_episodes : base_episodes;

        if (is_trauma)  current_phase = "Trauma";
        else if (is_therapy) current_phase = "Therapy";
        else current_phase = "Normal";

        std::cout << "\n┌────────────────────────────────────┐\n"
                  << "│ Lifetime " << (life + 1) << " / " << num_lifetimes 
                  << "  [" << current_phase << "] │\n"
                  << "└────────────────────────────────────┘\n";

        // Load from previous lifetime (skip on first)
        if (life > 0) {
            try {
        ...
        for (int ep = 0; ep < episodes_this_life; ++ep)
        {
            ...
            // Dynamic merge threshold — more aggressive later in therapy
            double merge_thresh = is_therapy ? 0.88 : 0.92;
            merge_thresh -= 0.01 * ep;
            agent.set_merge_threshold(merge_thresh);

            // Therapy phase adjustment
            if (is_therapy && ep >= 20) {
            ...
            for (int step = 0; step < 5; ++step)
            {
                ...
                bool is_trigger = (life > trauma_lifetime && discrete_state % 4 == 2);
                ...
                if (is_therapy) {
                    ...
                    reward_valence = (r < 0.40) ? -0.45 - dist(rng) * 0.20 
                                                : (r < 0.60 ? 0.06 : 0.02) + dist(rng) * 0.06;
                    if (r < 0.40) reward_surprise += 0.7;
                }
                else {
                    reward_valence = (r < mild_negative_prob) ? -negative_valence_range * dist(rng) 
                                                              : 0.04 + dist(rng) * 0.08;
                    if (r < mild_negative_prob) reward_surprise += 0.2;
                }
        ...
        // Checkpoint save every 5 lifetimes + always at the end
        if ((life + 1) % 5 == 0 || life == num_lifetimes - 1) {
            try {
                agent.save(save_file);
                std::cout << "Checkpoint saved after lifetime " << (life + 1) << "\n";
            } catch (const std::exception &e) {
    ...
    const int num_episodes = 100;          // increased for better statistics
